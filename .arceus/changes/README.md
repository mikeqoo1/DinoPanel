...
| `v0.6.12-nexus-write-enforcement`          | completed  | v0.6.12 | Quota bars alone mislead: CE enforces after a grace period, so an instance at 90% of the daily limit still rejects every push. Reads nexus_analytics_{blocked_requests_count,grace_throttled_requests,throttled_requests} (migration 0008), summarises them per queried range (blockedInRange distinguishes 'now' from 'historically') and banners it above the bars. Shipped 2026-09-18 |
| `v0.6.13-nexus-community-state`            | completed  | v0.6.13 | The guessed CE limits were both wrong (real: 100k requests/day, 40k components), so the panel showed 97% for an instance at 195%. Reads nexus.community.{usageLimits,throttlingStatus,gracePeriodEnds,*LastExceeded} from /service/extdirect/poll/rapture_State_get, caches per instance, and drives the banner from Nexus's own verdict instead of counter deltas scoped to the chart range. Shipped 2026-09-18 |
