...
const DF_GOLDEN = `檔案系統             1-區塊         已用        可用 容量 掛載點
tmpfs            1646391296      7602176  1638789120   1% /run
/dev/sda9      257802690560 166477746176 78154674176  69% /
tmpfs               5242880         4096     5238784   1% /run/lock
/dev/sda3         535805952      6979584   528826368   2% /boot/efi
`;
...
const DF_PSEUDO_GOLDEN = `Filesystem 1-blocks Used Available Capacity Mounted on
sysfs 0 0 0 - /sys
...
describe('parseDf', () => {
  it('parses df -PB1 rows positionally, skipping the (localized) header', () => {
    expect(parseDf(DF_GOLDEN)).toEqual([
      { source: 'tmpfs', mount: '/run', total: 1646391296, used: 7602176, available: 1638789120, usePercent: 1 },
      { source: '/dev/sda9', mount: '/', total: 257802690560, used: 166477746176, available: 78154674176, usePercent: 69 },
      { source: 'tmpfs', mount: '/run/lock', total: 5242880, used: 4096, available: 5238784, usePercent: 1 },
      { source: '/dev/sda3', mount: '/boot/efi', total: 535805952, used: 6979584, available: 528826368, usePercent: 2 },
    ]);
  });
  ...
  it('maps "-" capacity to null and skips non-numeric rows', () => {
    ...
    expect(rows).toEqual([
      { source: 'sysfs', mount: '/sys', total: 0, used: 0, available: 0, usePercent: null },
    ]);
  });

  it('clamps a >100% capacity (root-reserved blocks) to 100', () => {
    const rows = parseDf('Filesystem 1-blocks Used Available Capacity Mounted on\n/dev/sda1 1000 1000 0 105% /\n');
